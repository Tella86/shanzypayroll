...
// Calculate PAYE based on the gross salary
function calculatePAYE($gross_salary) {
    $paye = 0;
    
    // Step 1: 10% on the first KSh 24,000
    if ($gross_salary <= 24000) {
        return $gross_salary * 0.10;
    }
    
    $paye += 24000 * 0.10; // 10% of 24,000
    $remaining_salary = $gross_salary - 24000;

    // Step 2: 25% on income between KSh 24,001 and KSh 32,333
    if ($remaining_salary <= 8333) {
        return $paye + ($remaining_salary * 0.25);
    }
    
    $paye += 8333 * 0.25; // 25% of 8,333
    $remaining_salary -= 8333;

    // Step 3: 30% on income above KSh 32,333
    $paye += $remaining_salary * 0.30; // 30% of remaining income

    return $paye;
}

// Apply personal relief (KSh 2,400 per month)
function applyPersonalRelief($paye) {
    $personal_relief = 2400; // Current personal relief
    return max(0, $paye - $personal_relief);
}

// Calculate the gross salary and PAYE
$gross_salary = $payroll['gross_salary'];
$paye = calculatePAYE($gross_salary);
$paye_after_relief = applyPersonalRelief($paye);

// Add PAYE to deductions
$deductions['paye'] = $paye_after_relief;
